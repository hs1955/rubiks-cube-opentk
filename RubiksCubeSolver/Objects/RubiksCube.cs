...
using static RubixCubeSolver.Objects.RubiksCube.Turns;
using static RubiksCubeSolver.RubiksCubeSolver.StepState;
    ...
    class RubiksCube : CompositeGameObject
    {
        ...
        public bool ScrambleCube(float speed = 1.0f, bool hideCube = false)
        {
            ...
            if (isScrambleComplete)
            {
                ...
                List<int[]> allTurns = new List<int[]>();
            ...
            {
                Game.firstTime = true;
                RubiksCubeSolver.NetMenu.colors = getColors();
                setHide(false);
            }
        ...
        public bool SolveCube(float speed = 1.0f)
        {
            ...
            if (speed == 90.0f)
            {
                ...
                if (free)
                {
                    allTurns.Add(turnsToExcecute.ToArray());
                }
        ...
        bool free2 = true;
        bool reverse = false;
        Turns[] reverseTurns = new Turns[]
        {
            Ur, U, Dr, D, Rr, R, Lr, L, Fr, F, Br, B
        };
        public bool SolveStepByStep(StepState stepStateIn, float speedIn = 5.0f)
        {
            /// If not animating
            if (free2)
            {
                /// Decide what to do using stepStateIn
                reverse = false;
                speed2 = speedIn;
                switch (stepStateIn)
                {
                    ...
                    case NextStep:
                        ...
                        /// If the final step has been already been completed (stepNo is an 0-based index remember)
                        if (stepNo >= allTurns.Count)
                        {
                            /// Do nothing
                            return true;
                        }

                        /// If the index for which turn to carry out does NOT excedes the length of the array of the step (which is just batch of turns, since this is how the turns are stored)
                        if (turnNo < allTurns[stepNo].Length - 1)
                        {
                            /// Increment the index of the step to carry out by 1
                            turnNo++;
                        }

                        /// Else we are at the end of the step/batch of turns
                        else
                        {
                            /// Now start from the next step / next batch of turns
                            stepNo++;
                            turnNo = 0;
                        }
                        break;
                    ...
                    case PrevStep:
                        ...
                        /// We will have to invert the turn JUST made. 
                        /// This "JUST" is the reason for PrevStep not being an exact mirror of NextStep
                        /// The decrement in turnNo occurs after the turn is identified
                        ...
                        if (stepNo >= allTurns.Count || (stepNo <= 0 && allTurns[stepNo].Length == 0) || turnNo <= -1)
                        {
                            ...
                            /// else start from the end of the previous step
                            stepNo--;
                            turnNo = allTurns[stepNo].Length - 1; // -1 == last turn on previous step
                        }
                        break;
                }

                /// free2 is false now since animation must occur now.
                /// There's a chance this line is not needed, but this provides security anyways.
                free2 = false;
                
            }

            /// If there's turns to do
            if (stepNo >= 0 && stepNo < allTurns.Count && allTurns[stepNo].Length > 0)
            {
                /// The turn to excecute
                int turnToExcecute = allTurns[stepNo][turnNo];

                /// Do reverse turn instead if reverse flag raised
                if (reverse)
                {
                    turnToExcecute = (int)reverseTurns[turnToExcecute - 1];
                }

                /// Excecute Turn
                free2 = TurnSlice(turnToExcecute, speed2);

                /// Once the turn has just finished, if a Prev Turn was carried out
                if (reverse && free2)
                {
                    /// Decrement here
                    turnNo--;

                    /// Set reverse to false. Not sure if its necessary since it is handled above, but provides security, and makes it impossible for this to trigger again
                    reverse = false;
                }

            }

            /// A stage was completed, and this time was spent moving preparing for the next stage...
            else
            {
                /// ... so just remain free for animation. The switch(stepStateIn) should handle this and move to the next step
                free2 = true;
            }

            /// return the status of this function (because it's an animation, it must be called repeatedly until complete)
            return free2;
